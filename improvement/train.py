...
class DnCNN(torch.nn.Module):
    def __init__(self, depth=DEPTH, n_channels=64, image_channels=1):
        ...
        padding = 1
        layer1 = []
        layer1 += [nn.Conv2d(in_channels=image_channels, out_channels=n_channels, kernel_size=kernel_size, padding=padding),
                   nn.ReLU(inplace=True)]
        for i in range(depth-2):
            layer1 += [nn.Conv2d(in_channels=n_channels, out_channels=n_channels, kernel_size=kernel_size,padding=padding, bias=False),
                       nn.BatchNorm2d(n_channels, eps=0.0001, momentum=0.95),
                       nn.ReLU(inplace=True)]#卷积层后接由BatchNorm或者InstanceNorm层时，bias最好设为False
        layer1 += [nn.Conv2d(in_channels=n_channels, out_channels=image_channels, kernel_size=kernel_size, padding=padding, bias=False)]
        self.layer1 = nn.Sequential(*layer1)
        layer2 =[nn.Conv2d(in_channels=2*image_channels, out_channels=image_channels, kernel_size=1, padding=0, bias=False)]
        self.layer1 = nn.Sequential(*layer2)
        self._initialize_weights()
    def forward(self, x):
        y = x
        out0 = self.network(x)
        out = torch.cat([x, out0], 1)
        out = nn.Tanh(out)
        out = self.layer2(out)
        out = out*out0
        return y - out
